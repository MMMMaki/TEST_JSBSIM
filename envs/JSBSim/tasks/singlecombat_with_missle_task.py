...
class HierarchicalSingleCombatShootTask(HierarchicalSingleCombatTask, SingleCombatShootMissileTask):
    ...
    #         return np.zeros(len(self.action_var))
    ...
    #     # 【终极修复】：保护 Baseline 靶机的动作不被 RL 的占位符覆盖
    #     if action is None or (getattr(self, 'use_baseline', False) and agent_id in env.enm_ids):
    #         self._shoot_action[agent_id] = 0
    #         # 直接提取并返回红方当前的真实底层舵面数据，完美骗过外层覆盖机制！
    #         return env.agents[agent_id].get_property_values(self.action_var)
    ...
    #     # 【核心修复】：拦截红方靶机，直接计算并应用其连续物理动作
    #     if 'Baseline' in getattr(env, 'config_name', '') and agent_id in env.enm_ids:
    #         # 动态挂载靶机大脑 ('n' 代表常规机动规避)
    #         if not hasattr(self, 'baseline_agent'):
    #             from envs.JSBSim.model.baseline import ManeuverAgent
    #             self.baseline_agent = ManeuverAgent(agent_id=1, maneuver='n')
            
    #         # 独立获取红方的 21 维观测数据
    #         enm_obs = self.get_obs(env, agent_id)
            
    #         # 靶机模型需要字典索引格式
    #         raw_continuous_action = self.baseline_agent.get_action({1: enm_obs})
            
    #         # 靶机只负责逃跑，不射击
    #         self._shoot_action[agent_id] = 0 
            
    #         # 重点：直接返回连续浮点数，绝对不要进行 int32 强转！
    #         return raw_continuous_action

    #     # --- 以下是原逻辑，仅为蓝方 RL 智能体的离散动作服务 ---
    #     if action is None: 
    #          self._shoot_action[agent_id] = 0
    #          return np.zeros(len(self.action_var))
    ...
    # def normalize_action(self, env, agent_id, action):
    #     # 拦截空动作或靶机动作
    #     if action is None or ('Baseline' in getattr(env, 'config_name', '') and agent_id in env.enm_ids):
    #         if not hasattr(self, 'baseline_agent'):
    #             from envs.JSBSim.model.baseline import ManeuverAgent
    #             self.baseline_agent = ManeuverAgent(agent_id=1, maneuver='n')
            
    #         # 获取靶机观测，包装成字典骗过 baseline.py 的索引检查
    #         enm_obs = self.get_obs(env, agent_id)
    #         raw_continuous_action = self.baseline_agent.get_action({1: enm_obs})
            
    #         self._shoot_action[agent_id] = 0 
    #         return raw_continuous_action # 直接返回连续动作，不取整！
    ...
    def normalize_action(self, env, agent_id, action):
        ...
        # 拦截靶机动作，直接调用它内置的大脑
        if action is None or ('Baseline' in getattr(env, 'config_name', '') and agent_id in env.enm_ids):
            # 划重点：靶机大脑期望接收的是底层的 sim 对象，而不是观测字典！
            sim = env.agents[agent_id]
            
            # 直接获取连续动作
            raw_continuous_action = self.baseline_agent.get_action(sim)
